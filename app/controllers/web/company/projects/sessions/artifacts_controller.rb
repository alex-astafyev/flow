...
class Web::Company::Projects::Sessions::ArtifactsController < Web::Company::Projects::ApplicationController
  def index
  ...
  def review
    session = current_project.terminal_sessions.find(params[:session_id])
    authorize_session_visibility!(session)
    ...
    redirect_to company_project_session_path(current_project, session), notice: "Artifacts reviewed successfully"
  end
end
