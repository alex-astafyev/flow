...
> Variables live in `.env.development` for local dev, `.env.production`
> for production. Anything that's a secret should *never* be committed.
...
> Generate with `bundle exec rails secret`. Rotate by re-encrypting
> stored values — there's no online rotation primitive yet.
